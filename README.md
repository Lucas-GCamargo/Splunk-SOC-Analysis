...
Kaggle Dataset (CSV)
        ↓
Manual upload to Splunk → index=security (Settings → Add Data)
        ↓
...
```
> Phase 1 was completed manually to perform the initial analysis and build the dashboard. Phase 2 (`splunk_ingest.py`) is a separate automation tool built to keep the dashboard continuously updated in production — eliminating the need for manual CSV uploads.
...
**Recommendation:** Review and update firewall/IPS rules so all High and Critical severity events trigger an automatic block and generate an alert — currently they are being Ignored at the same rate they are Blocked.
...
|---|---|
| **SPL (Search Processing Language)** | 5 production-ready queries covering aggregation, time-series analysis, anomaly detection, data cleaning, and alerting |
